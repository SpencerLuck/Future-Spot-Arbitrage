#!/usr/bin/env python3
"""
Binance SPOT kline downloader (importable + CLI).

- Auto-discovers SPOT symbols (status=TRADING) filtered by quote asset (default: USDT)
- Resume-safe CSVs per symbol+interval: <symbol>_<interval>.csv
- Start/end time windowing with paging via startTime/endTime/limit
- Same function/CLI style as `fetch_perpetual_klines` for easy side-by-side use
"""
...
def csv_path_for(symbol: str, interval: str, out_dir: str) -> str:
    ...
    return os.path.join(out_dir, f"{symbol}_{interval}_SPOT.csv")
...
def get_spot_symbols(
    *,
    ...
    """
    Return list of SPOT symbols that are TRADING.
    If quote_asset is provided (default 'USDT'), only include those pairs.
    """
    ...
    for s in data.get("symbols", []):
        ...
        if not s.get("isSpotTradingAllowed", True):
            continue
        if quote_asset and quote_asset.upper() != "ANY":
            if s.get("quoteAsset") != quote_asset.upper():
                continue
        syms.append(s["symbol"])
    return sorted(syms)

def iter_spot_klines_range(
    symbol: str,
    *,
    start_ms: int,
    end_ms: Optional[int],
    ...
    """
    Yield spot klines from start_ms (inclusive) to end_ms (exclusive) in chronological order,
    fetching in pages of `limit` (spot max 1000). If end_ms is None, fetch until 'now'.
    """
...
# ---- Public callable ---------------------------------------------------------
@dataclass
class FetchSpotConfig:
    # selection
    symbols: Optional[List[str]] = None
    interval: str = "1m"
    limit: int = 1000
    quote_asset: Optional[str] = "USDT"   # filter auto-discovery; set to "ANY" for all
    ...
    # io + behavior
    out_dir: str = os.path.join("arbitrage", "data_spot")  # default separate directory
    resume: bool = True
    pause: float = 0.12
    pause_between_symbols: float = 0.25
    retries: int = 5
    user_agent: str = "binance-spot-1m/0.3"
    base_url: str = SPOT_BASE_DEFAULT
    ...
    def apply_env_defaults(self):
        ...
        qa = env.get("QUOTE_ASSET")
        if qa:
            self.quote_asset = qa

def fetch_spot_klines(
    symbols: Optional[List[str]] = None,
    out_dir: str = os.path.join("arbitrage", "data_spot"),
    interval: str = "1m",
    lookback_minutes: int = 60 * 24 * 30,
    start_ms: Optional[int] = None,
    end_ms: Optional[int] = None,
    ...
    """
    Download SPOT klines for given symbols to CSVs.

    - If `symbols` is None/empty, auto-discovers TRADING spot pairs (filtered by `quote_asset`).
    ...
    if not cfg.symbols:
        if cfg.verbose:
            print(f"Discovering SPOT symbols (quote={cfg.quote_asset or 'ANY'})...")
        cfg.symbols = get_spot_symbols(
            base=cfg.base_url, user_agent=cfg.user_agent, insecure_ssl=cfg.insecure_ssl,
            retries=cfg.retries, pause=cfg.pause, quote_asset=cfg.quote_asset
        )

    if cfg.verbose:
        print(f"Found {len(cfg.symbols)} SPOT symbols")
    ...
    for i, sym in enumerate(cfg.symbols, start=1):
        try:
            ...
            if end_ms_sym is not None and start_ms_sym is not None and start_ms_sym >= end_ms_sym:
                if cfg.verbose:
                    print(f"[{i}/{len(cfg.symbols)}] {sym}: up-to-date, skipping")
                rows_written = 0
            else:
                if cfg.verbose:
...
def _parse_args():
    import argparse
    p = argparse.ArgumentParser(description="Download Binance SPOT klines to CSV.")
    p.add_argument("--symbols", type=str, default=None,
                   help="Comma-separated symbols (e.g., BTCUSDT,ETHUSDT). If omitted, auto-discover TRADING pairs.")
    p.add_argument("--quote-asset", type=str, default="USDT",
                   help="Filter auto-discovered symbols by quote asset (default: USDT). Use 'ANY' for all.")
    p.add_argument("--out-dir", type=str, default=os.path.join("arbitrage", "data_spot"))
    ...
    p.add_argument("--user-agent", type=str, default="binance-spot-1m/0.3")
    p.add_argument("--base-url", type=str, default=SPOT_BASE_DEFAULT)
